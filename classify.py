...
z = tf.placeholder(tf.float32, (None ,32, 32,1))
...
def preprocessed(data):
    ...
    Applying the preprocessing steps to the input data.
        Parameters:
            data: An np.array compatible with plt.imshow.
    """
    gray_images = gray_scale(data)
    equalized_images = local_histo_equalize(gray_images)
    n_training = data.shape
    normalized_images = image_normalize(equalized_images)
    return normalized_images
...
def y_predict_model(Input_data, top_k=5):
    ...
    with tf.Session() as sess:
        ...
        y_prob, y_pred = sess.run(tf.nn.top_k(tf.nn.softmax(VGGNet_Model.logits), k=top_k),
                             feed_dict={z:Input_data, keep_prob:1, keep_prob_conv:1})
